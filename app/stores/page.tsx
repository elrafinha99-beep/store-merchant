...
export default function Page() {
  ...
  const getData = async (currentPage: number, searchTerm: string) => {
    ...
    try {
      let q = supabase
        .from('stores')
        .select('id, name, city, phone_number', { count: 'exact' })
        .filter('active', 'eq', true)
        .order('name')
        .range(from, to)

      if (searchTerm.trim()) {
        q = q.or(`name.ilike.%${searchTerm}%,city.ilike.%${searchTerm}%,phone_number.ilike.%${searchTerm}%`)
      }
        ...
        {loading ? (
          ...
          <>
            <DataTable
              columns={columns}
              data={stores}
              onRowClick={(x) => router.push('/stores/' + x.id)}
            />
            ...
            {totalCount > 0 && (
              <div className="flex items-center justify-between mt-4">
                <p className="text-sm text-gray-500">
                  ...
                  <span className="font-medium">{totalCount}</span> stores
                </p>
